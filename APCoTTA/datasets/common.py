...
class PointCloudDataset(Dataset):
    ...
    def Strongaugmentation_transform(self, points, normals=None, verbose=False):
        """Implementation of an augmentation transform for point clouds."""
        ...
        if points.shape[1] == 3:
            if self.config.augment_rotation2 == 'vertical':
            ...
            elif self.config.augment_rotation2 == 'all':
        ...
        # Choose random scales for each example
        min_s = self.config.augment_scale_min2
        max_s = self.config.augment_scale_max2
        if self.config.augment_scale_anisotropic2:
            scale = np.random.rand(points.shape[1]) * (max_s - min_s) + min_s
        else:
            scale = np.random.rand() * (max_s - min_s) + min_s

        # Add random symmetries to the scale factor
        symmetries = np.array(self.config.augment_symmetries2).astype(np.int32)
        symmetries *= np.random.randint(2, size=points.shape[1])
        scale = (scale * (1 - symmetries * 2)).astype(np.float32)
        ...
        noise = (np.random.randn(points.shape[0], points.shape[1]) * self.config.augment_noise2).astype(np.float32)
