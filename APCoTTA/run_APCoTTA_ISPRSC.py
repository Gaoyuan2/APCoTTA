...
def SelectADAPTATION(base_model,optimizer,optimizer2,net2,config,modelName):
...
def Prepare_parameter():
    chosen_log='results//ISPRS_checkpoint'
...
def Prepare_model(config,chkp_path,label_values,ignored_labels):
    ...
    if config.dataset_task == 'classification':
        ...
        net2 = KPFCNN(config, label_values, ignored_labels)
    else:
    ...
    deform_params2 = [v for k, v in net2.named_parameters() if 'offset' in k]
    other_params2 = [v for k, v in net2.named_parameters() if 'offset' not in k]
    ...
    optimizer2 = torch.optim.SGD([{'params': other_params2},
                                 {'params': deform_params2, 'lr': deform_lr}],
                                lr=config.learning_rate,
                                momentum=config.momentum,
                                weight_decay=config.weight_decay)
    ...
    return net,device,optimizer,optimizer2,net2
...
def cloud_segmentation_test(net, test_loader, config,device ,ioupath,testpath, num_votes=10,debug=False):
        ...
        with torch.no_grad():
            for i, batch in enumerate(test_loader):
                ...
                '''判断是否所有点都预测label'''
                if all(np.all(subarray>-1) for subarray in test_probs):
                    break
...
if __name__ == '__main__':
    ...
    # path=r'/data/ALS_CTTA_KUA/ISPRSCTTA'
    path=r'data/H3DC'
    ...
    ignored_labels = np.array([9])

    net,device,optimizer,optimizer2,net2=Prepare_model(config,chosen_chkp,label_values,ignored_labels)



    mode='our'
    message='Ablation22222'
    config.validation_size = 2000
    config.input_threads = 10
    config.datasetClass = 'ISPRS'

    net=SelectADAPTATION(net,optimizer,optimizer2,net2,config,modelName=mode)

    ioufile_path = os.path.join(path, 'ALLtest_'+mode+'_'+message+'_.txt')
    ...
    for ij in tqdm(range(len(datasetIndex)),desc='*JinDuing********'):
        ...
        if config.dataset_task == 'cloud_segmentation':
            miou,oa,dname=cloud_segmentation_test(net, test_loader, config,device,ioufile_path,mode+'_'+message)
