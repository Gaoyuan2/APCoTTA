...
Sunlight
'''
def scene_glare_noise(pointcloud,label, ignorelabel=9,severity=5):
...
def gaussian_noise(pointcloud,label, severity):
    ...
    jitter = np.random.normal(size=(N, C)) * c*1.001
...
def uniform_noise(pointcloud, label,severity):
    ...
    N, C = pointcloud.shape
    c = [0.02, 0.04, 0.06, 0.08, 0.10][severity - 1]
    jitter = np.random.uniform(-c, c, (N, C))*1.001
    new_pc = (pointcloud + jitter).astype('float32')
    return new_pc,label
...
3D noise
'''
def Spacenoise(point_cloud,label, dataclassname,ignorelabel=9,levels=5):
    #噪声点数，用于控制噪声水平
