...
from utils.wet_ground_Robo3D.planes import calculate_plane
from  utils.wet_ground_Robo3D.phy_equations import total_transmittance_from_ground


def ground_water_augmentation(pointcloud, label,datesetName, water_height=0.0012, pavement_depth=0.0012, noise_floor=0.25, power_factor=15,
    ...
    if datesetName=='isprs':
        ground=label==2 #imprervious surfaces
        ground_idx = np.where(ground)
        pointcloud_planes = pointcloud[ground, :]
    ...
    pointcloud_planes = pointcloud_planes[:, :]
    ...
    augmented_label = np.zeros((pointcloud.shape[0] - ground_idx[0].shape[0] + keep_points_idx[0].shape[0], ))
    augmented_label[:pointcloud.shape[0] - ground_idx[0].shape[0]] = label[np.logical_not(ground)]
    augmented_label[pointcloud.shape[0] - ground_idx[0].shape[0]:] = label[ground_idx[0][keep_points_idx]]
